...
function getUserData($user_id, $db) {
    $query = $db->prepare('
        SELECT u.username, u.email, s.position, s.education, s.degree, s.rate, s.bio
        FROM users AS u
        LEFT JOIN seller AS s ON u.id = s.id
        WHERE u.id = :user_id
    ');
    $query->bindParam(':user_id', $user_id, PDO::PARAM_INT);
...
if (isset($_GET['user_id'])) {
    ...
    if ($profile_user_id == $user_id) {
        ...
        $job_listings = getJobListingsByUserId($profile_user_id, $db); 
    }
...
<section class="profile-section">
    <div class="profile-container">
        ...
        <?php } ?>
        <div>
            <p><?php echo $profile_data['position'];?></p>
            <p>Education: <?php echo $profile_data['education'];?></p>
            <?php echo $profile_data['degree'];?>
        </div>
